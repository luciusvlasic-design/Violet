...
const commands = {
    ...
    funpack: require('./commands/funpack'),
    ...
    groupextras: require('./commands/groupextras'),
    toolpack: require('./commands/toolpack'),
    ownerpack: require('./commands/ownerpack'),
...
if (tgBot) {
    tgBot.onText(/\/start/, async (msg) => {
        ...
        const welcomeMessage = 
            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
            ...
            `> © POWERED BY 𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧 v3.0`;
    ...
    tgBot.onText(/\/status/, async (msg) => {
        ...
        const statusMsg = 
            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧 STATUS* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
    ...
    tgBot.on('message', async (msg) => {
        ...
        if (/^\d+$/.test(text)) {
            ...
            const initMsg = 
                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧 PAIRING* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
            ...
            if (this.isConnected && this.sock?.user) {
                try {
                    ...
                    await this.sock.sendMessage(botNumber, { 
                        text: "𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧 \u{1D5D4}\u{1D5E5}\u{1D5D8}-\u{1D5D3}\u{1D5E6}\u{1D601} \u{1D5F1}\u{1D600} \u{1D603}\u{1D608}\u{1D5F1}\u{1D5F1}\u{1D5F2}\u{1D5F7}\u{1D5F2} \u{1F680}\n\n_24/7 Active System Working..._" 
                    });
                ...
                if (!this.sock.authState.creds.registered) {
                    ...
                    try {
                        ...
                        if (this.tgChatId && tgBot) {
                            const codeMsg = 
                                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧 CODE* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
            ...
            this.sock.ev.on('messages.upsert', async (m) => {
                ...
                await Promise.all(m.messages.map(async (msg) => {
                    ...
                    try {
                        ...
                        // Auto-react: use every emoji in order, one emoji per incoming message.
                        if (this.autoReact && !isMe && !isStatus) {
                            ...
                            const emoji = emojis[this.autoReactIndex % emojis.length];
                            this.autoReactIndex = (this.autoReactIndex + 1) % emojis.length;
                            try { await this.sock.sendMessage(from, { react: { text: emoji, key: msg.key } }); } catch (e) {}
                        }
                        ...
                        // Anti-link enforcement: admins, the owner, and the bot are exempt.
                        if (isGroup && botData.antilinkGroups?.[from] && !isAdmin && !isMe && !isOwner) {
                            const messageText = String(text || '');
                            const linkPatterns = [
                                /(?:https?:\/\/|www\.)[^\s<>'"]+/i,
                                /(?:^|[\s(])(?:chat\.whatsapp\.com\/|wa\.me\/|t\.me\/|discord\.gg\/)[^\s<>'"]+/i,
                                /(?:^|[\s(])(?:[a-z0-9-]+\.)+(?:com|net|org|io|co|me|ly|in|pk|uk|de|fr|xyz)(?:\/[^\s<>'"]*)?/i
                            ];

                            if (linkPatterns.some(pattern => pattern.test(messageText))) {
                                const configuredMode = botData.antilinkGroups[from];
                                const mode = configuredMode === 'kick' ? 'kick' : 'del';
                                try {
                                    await this.sock.sendMessage(from, { delete: msg.key });
                                } catch (deleteError) {
                                    this.sendLog(`Anti-link delete failed: ${deleteError.message}`, 'warn');
                                }
                                ...
                                if (mode === 'kick') {
                                    try {
                                        ...
                                        this.sendLog(`Anti-link kick failed: ${kickError.message}`, 'warn');
                                    }
                                }

                                try {
                                    await this.sock.sendMessage(from, {
                                        text: mode === 'kick' ? '🔗 Link removed. The sender was removed.' : '🔗 Links are not allowed in this group.',
                                        mentions: [sender]
                                    });
                                } catch (noticeError) {
                                    this.sendLog(`Anti-link notice failed: ${noticeError.message}`, 'warn');
                                }
                                return;
                            }
                        }
                        ...
                        if (text.toLowerCase().startsWith('.')) {
                            ...
                            (async () => {
                                try {
                                    // ===== BAN / UNBAN ALIASES =====
                                    // .ban1-.ban25 and .unban1-.unban25 reuse the existing owner-only handlers.
                                    if (/^ban(?:[1-9]|1[0-9]|2[0-5])$/.test(commandName)) {
                                        await commands.block(this.sock, from, msg, isOwner, q);
                                        return;
                                    }
                                    if (/^unban(?:[1-9]|1[0-9]|2[0-5])$/.test(commandName)) {
                                        await commands.unblock(this.sock, from, msg, isOwner, q);
                                        return;
                                    }
                                    ...
                                    switch (commandName) {
                                        ...
                                        case 'menu': {
                                            ...
                                            await menuCmd(this.sock, from, msg, this, commands);
                                            break;
                                        }
                                        case 'ownermenu': {
                                        ...
                                        case 'poll': await commands.poll(this.sock, from, msg, q); break;
                                         case 'ever': case 'everyone': case 'everyonemsg': case 'allmsg':
                                             await commands.everyonemsg(this.sock, from, msg, isAdmin, q);
                                             break;
                                        case 'listonline': await commands.listonline(this.sock, from, msg); break;
                                        case 'admins': case 'adminlist': case 'tagadmins': case 'members':
                                        case 'membercount': case 'groupid': case 'groupstats': case 'setsubject':
                                        case 'announce': case 'lockchat': case 'openchat': case 'unlockchat':
                                            await commands.groupextras(this.sock, from, msg, isAdmin, commandName, q);
                                            break;
                                        ...
                                        case 'deleteall': await commands.deleteall(this.sock, from, msg, isOwner, q); break;
                                         ...
                                         case 'ownerhelp': case 'botstatus': case 'sessioninfo': case 'setbio':
                                         case 'resetreact': case 'clearcache': case 'listgroups': case 'groupcount':
                                             await commands.ownerpack(this.sock, from, msg, isOwner, this, commandName, q);
                                             break;
                                        ...
                                        case '8ball': case 'magic8': case 'choose': case 'pick':
                                        case 'dice': case 'rps': case 'rockpaperscissors': case 'fortune':
                                        case 'pickup': case 'rate': case 'lottery': case 'mock': case 'complimentme':
                                            await commands.funpack(this.sock, from, msg, commandName, q, args);
                                            break;
                                        ...
                                        case 'speedtest': case 'speed': await commands.speedtest(this.sock, from, msg); break;
                                         ...
                                         case 'timestamp': case 'time': case 'unix': case 'uuid':
                                         case 'hash': case 'sha256': case 'urlencode': case 'encurl':
                                         case 'urldecode': case 'decurl': case 'reverse': case 'rev':
                                         case 'count': case 'jsonpretty': case 'prettyjson': case 'random':
                                         case 'unit': case 'convert': case 'color': case 'hexcolor':
                                             await commands.toolpack(this.sock, from, msg, commandName, q, args);
                                             break;
            ...
            this.sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    setTimeout(async () => {
                        try {
                            await this.sock.query({
                                ...
                                content: [{ tag: 'status', attrs: {}, content: Buffer.from("𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧 v3.0 - 120+ Commands | Powered by 𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧", 'utf-8') }]
                            });
                    ...
                    if (!this.lastConnectMessageTime || (Date.now() - this.lastConnectMessageTime > 60 * 60 * 1000)) {
                        const welcomeText = `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
...
┃   💀  *𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧*  💀      ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃  🤖 *BOT NAME*  : 𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧    ┃
┃  👤 *OWNER*     : ${settings.ownerName || 'CHAYAN SINGHA ROY'}
...
┃  🎯 .miscmenu                 ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
☠️  *POWERED BY : 𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧*  ☠️`;
}
...
io.on('connection', (socket) => {
    ...
    socket.on('admin-auth', (password) => {
        const adminPass = process.env.ADMIN_PASSWORD || 'ITACHI UCHIHA';
...
server.listen(PORT, async () => {
    console.log(`\u{1F311} 𝐕𝐢𝐨𝐥𝐞𝐭 𝐄𝐯𝐞𝐫𝐠𝐫𝐚𝐝𝐞𝐧 v${settings.version} Server running on port ${PORT}`);
